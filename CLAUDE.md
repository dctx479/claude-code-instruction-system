...
8. **🆕 外部工具推荐检查 (v1.1)**: 提取 `tool_recommendation` 字段
 - 字段为 `null` → 跳过此步骤
 - 字段非空 → 它是对象 `{tool, priority, install_cmd, verify_cmd, expected, doc}`
 - **执行工具安装验证**: 用 `Bash` 工具运行 `verify_cmd`，判断工具是否已安装
 - **已安装**（验证通过）→ 静默继续，直接用该工具完成任务
 - **未安装**（验证失败/命令不存在）→ **必须主动引导用户安装**，参考下方「外部工具引导话术模板」
 - **触发频率**: 本会话内首次检测到该工具的推荐时引导一次；用户拒绝后不再重复
9. 步骤 2（提取 agent）和步骤 8（验证工具）可并行：先并行发出 1 个 Read + 1 个 Bash 调用，再综合决策

#### 外部工具引导话术模板

当 `tool_recommendation` 非空且工具未安装时，回复中**必须**包含以下结构（**禁止直接动手安装**）：

> 我检测到你需要 [触发场景的简短描述]。根据系统配置，建议安装 **[工具名称]**，可 [预期效果的一句话]。
>
> **安装命令**: `<install_cmd>`
> **验证方法**: 运行 `<verify_cmd>` 预期看到：<expected>
>
> **是否立即安装？** 安装后我将自动用该工具 [具体动作]。

**禁止**：
- ❌ 不经用户同意直接执行 install_cmd（属于 §七「需要确认」的范围）
- ❌ 用模糊话术（"考虑装个工具"），必须给出明确的 install_cmd 和 verify_cmd
- ❌ 跳过验证步骤（必须确认未安装后才推荐）

**允许**：
- ✅ 用户口头同意（「装吧」）后直接执行 install_cmd 并验证
- ✅ 多个工具同时需要时一次性列出（如需要 Context Mode + CodeGraph）
- ✅ 已在 CLAUDE.md §十二 授权范围：用户口头许可即可执行（推荐外部工具安装）
...
| codemap | codemap-builder | `agents/codemap-builder.md` | 代码地图 - 代码库导航文档 |
| data-warehouse | data-warehouse-analyst | `agents/data/data-warehouse-analyst.md` | 数据仓库分析 - BI/数仓查询 |
...
**检测到学习触发 → 先写入 memory/ → 再回复用户**。理由：context 最鲜活的时刻是事件发生那一刻，事后补写细节丢失。

写入前 Grep 搜索已有条目的 Pattern-Key，匹配则更新 Recurrence-Count，不匹配则新建。

> 详见: `memory/best-practices.md` BP-029（五断裂点防治）、BP-035（假闭环诊断）

### 主动自我纠正触发条件

当以下任一条件出现时，**必须主动显式声明**，不等用户追问：

| 触发条件 | 声明格式 | 后续动作 |
|---------|---------|---------|
| 方案与原始需求矛盾 | "⚠️ 我意识到方案 X 与原始需求 Y 冲突，具体是..." | 停，等用户确认 |
| 缺少关键信息做了假设 | "⚠️ 我在没有确认 Z 的情况下做了假设：如果 Z 是...则以上结论成立" | 停，等用户确认 |
| 与之前结论冲突 | "⚠️ 我之前说...，但现在发现...（证据），可能之前的结论有问题" | 说明冲突，要求重审 |
| 超出能力边界 | "⚠️ 这个问题需要...（具体专业知识），我只能给出一般性建议" | 建议找专家或工具 |

**原则**：保持诚实胜过保持一致。发现可能错误时，主动声明比等用户发现再纠正代价低得多。

> 详见: `memory/best-practices.md` BP-026（CLAUDE.md 规则提炼原则）
...
8. **收尾整理**：调用 `/neat`（洁癖 Skill）同步 docs/CLAUDE.md/memory 三层知识，防止失同步

**何时触发**：autopilot/ralph 结束、解决重大 bug、完成新功能开发、批量任务完成后

### /neat 收尾审查（强烈建议）

任何复杂任务收尾时，调用 `/neat` 执行五步法清理：
1. **机械盘点**：Read 所有受影响 md 文件
2. **影响矩阵**：识别 docs/CLAUDE.md/memory 哪一层需要更新
3. **按层修改**：docs → CLAUDE.md → memory（核心原则：合并优于追加，删除优于保留）
4. **自检清单**：环境变量、相对时间、悬空引用、规模上限
5. **变更摘要**：输出改前→改后对照

> 详见 `.claude/skills/neat/SKILL.md`。/compact 之前、关闭窗口之前、autopilot/ralph 结束时强烈建议触发。
...
| **任务难度多变** | **ADAPTIVE** | **智能路由 + 自组织** |

**ADAPTIVE 模式**：任务复杂度不可预测时，给 Agent 提供可用工具集和目标约束，让其自行选择模型档位（mini/sonnet/opus）和组合 Skills。成本优化可达 16-50 倍。

**Agent 按模型分级**：决策层（architect）用 Opus + 全权限；执行层（debugger）用 Sonnet + 受限权限；监控层（performance-monitor）用 Haiku + 只读权限。详见 `docs/ORCHESTRATION-GUIDE.md` § Agent 按模型分级策略。

**任务地图**：复杂任务先明确主 Agent、辅助 Agent、任务依赖、交付物和验收证据，再进入并行或顺序执行。
...
**铁律：No Spec, No Code — 没有规范文档，不动手写代码。**
...
**失败归因**：任务失败时必须分清原因（需求不明确 / Spec 错误 / Plan 执行偏差 / 外部依赖），归因后必须制定预防措施并写入对应文件（详见 `memory/best-practices.md` BP-028）。

**任务终止标准**：测试通过 ≠ 完成。必须同时满足：需求对齐 + 验收通过 + 质量达标（≥80分）+ 结果同步 + 未验证项已声明（详见 `agents/qa-reviewer.md`）。

**知识资产审计**（每季度一次）：Skill 激活率目标 ≥50%，Spec 复用率目标 ≥30%，详见 `memory/best-practices.md` BP-027。

**知识资产利用率审计**（每季度一次）：Skill 激活率目标 ≥50%，Spec 复用率目标 ≥30%，详见 `memory/best-practices.md` BP-027。

**能力路由**: 先判断任务类型、证据源、权限边界和 SNR，再加载 Agent / Skill / 工具；详见 `docs/CONTEXT-ENGINEERING-GUIDE.md` 与 `workflows/routing/intent-matcher.md`。

**会话管理规则**:
- 新任务 → 新会话 | 失败尝试 → /rewind 重试而非纠正
- /compact 时附上未来方向说明（防坏压缩）| Subagent 隔离大量中间输出
- **/compact 之前 / 关闭窗口之前**：调用 `/neat` 执行三层知识同步（docs/CLAUDE.md/memory），防止跨会话失同步

> 详细指南: `docs/CONTEXT-ENGINEERING-GUIDE.md`

---

## 六.六、AI 协作核心行为约束（Karpathy 规则）

> 这四条规则经 17w+ Star 验证，是 AI 行为约束的最小有效集合。详见 `memory/best-practices.md` BP-046
...
### 废弃规则管理（Deprecation Protocol）

过时规则**标记废弃而非直接删除**，保留 60 天后方可移除。

**废弃标记格式**：

```markdown
> [!NOTE] DEPRECATED 2026-06-10  
> 已废弃，替代方案：[新规则位置或描述]
...
**适用场景**：
- `deprecate`（标记）：曾经正确但现在过时、有历史参考价值
- `delete`（直接删除）：错误记录、重复条目、无价值噪声

**废弃生命周期**：标记日 → 60 天保留期 → 季度审计时清除

> 详见: `memory/best-practices.md` BP-026（规则提炼）、BP-037（事故驱动增长）

### 规则增长策略（事故驱动）

**只根据事故继续加规则**。不是因为"看起来不够完整"就继续加，而是因为某类错误反复出现（≥2次），所以专门补一条下一次能执行的约定。

CLAUDE.md 是保障栈最轻的一层（L0 软提醒），不能替代 lint(L1)、测试(L2)、hooks(L3)、CI/Review(L4)。规则放对层级比多写规则更重要。

> 详见: `memory/best-practices.md` BP-036（防御纵深）、BP-037（事故驱动规则增长）
...
**核心区分**：
- **Cron（时间驱动）**：因为时间流动 → 定期做某事（"每小时检查一次部署状态"）
- **Webhook（事件驱动）**：因为世界变了 → 状态改变时立刻响应（"PR 被打开时立刻审查"）

**决策树**：
```
需要响应外部状态变化? ─YES→ Webhook（PreToolUse / PostToolUse / Stop Hook）
    NO↓
需要定期执行不依赖外部触发? ─YES→ Cron（CronCreate）
    NO↓
一次性延迟任务? ─YES→ ScheduleWakeup
```
...
| 场景 | 类型 | 实现方式 | 示例 |
|------|------|---------|------|
| 每天早上 9 点检查待办 | 时间驱动 | CronCreate | `CronCreate("0 9 * * *", "检查今日任务")` |
| GitHub PR 被打开 | 事件驱动 | Webhook + PreToolUse | GitHub MCP tool → 触发 Hook → 自动 review |
| 文件保存时自动格式化 | 事件驱动 | PostToolUse Hook | Write tool 完成 → Hook → 运行 prettier |
| Notion 任务拖到"待拍摄" | 事件驱动 | 轮询（模拟事件）| 每 5 分钟扫描变化 + 状态比对 |
| Ralph 循环每轮结束 | 事件驱动 | Stop Hook | round_complete=true → 递增 iteration |

**设计原则**：
- 外部系统主动推送事件 → Webhook（真事件驱动）
- 外部系统不支持推送但状态会变 → 短周期 Cron 轮询（模拟事件驱动）
- 纯内部定时任务 → Cron（时间驱动）

> 详见: `hooks/hooks.json`, `commands/general/ralph.md` (Stop Hook 示例)
...
| Paper Writing Assistant | 综述生成、论文撰写、风格学习、Rebuttal、论文转演示 |
| Experiment Logger | 结构化记录、参数管理、结果对比 |
| Data Analyst | 数据预处理、统计分析（含检验选择）、可视化 |

**论文全生命周期**：找方向 → 研究方案 → 实验 → 写作 → 引用审计 → 投稿前 QA → Rebuttal → 演示交付
...
| 想跨工具统计 token 总用量 / 多个 AI 工具并用 | **Tokscale** | `npx tokscale@latest` |
| Agent 工具输出/RAG 结果/日志大量堆积（Context Mode + RTK 已装） | **Headroom** | `pip install "headroom-ai[all]"` |
| Agent 需要操作浏览器 / 网页自动化 / 数据抓取 | **Camoufox** | `pip install -U "camoufox[geoip]" && python -m camoufox fetch` |
...
| 用户做前端开发/想消除 AI 模板感/要高质量 UI | **taste-skill** | `npx skills add Leonxlnx/taste-skill -g -y` |
| 用户做渗透测试 / CTF / 安全研究 | **hack-skills** | `npx skills add yaklang/hack-skills` |
