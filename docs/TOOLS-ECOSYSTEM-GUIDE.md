...
| **Headroom** | Token 优化 | Agent 全链路输入预压缩，CCR 可逆，跨 Agent 记忆 | ⭐ pip install | Agent 工具输出/日志/RAG 大量堆积 |
| **Tokscale** | 可观测性 | 跨 20+ AI 编程工具 token 用量汇总可视化 | ⭐ 零安装 npx | 想跨工具统计 token 总用量 |
| **Camoufox** | 浏览器自动化 | Firefox 指纹伪装，AI Agent 专用浏览器，Playwright 兼容 | ⭐⭐ pip + fetch | Agent 需要操作浏览器/网页自动化 |
...
| 组合 | 覆盖范围 | 预期效果 | 推荐场景 |
|------|---------|---------|---------|
| **RTK 单独** | Bash/外部命令 | 60-90% | **🥇 首次安装优先 — 立竿见影** |
| **Context Mode 单独** | MCP 工具输出 | ~98% 输出压缩 | 频繁使用 MCP 工具（Playwright、GitHub、DB） |
| **CodeGraph 单独** | 代码理解 | ~92% 调用减少 | 大型代码库探索和重构 |
| **RTK + Context Mode** | Bash + MCP | 双层压缩 | 推荐日常组合 |
| **Context Mode + CodeGraph** | MCP + 代码 | 双管齐下 | 重代码探索场景 |
| **三合一** | 全覆盖 | 最大化节省 | 长时间会话、复杂多工具任务 |
| **四合一 + Headroom** | 全链路（含 RAG/日志/对话历史） | 极致压缩 | 极端复杂 Agent 场景 |

**首次安装优先级推荐**：
1. **RTK**（5 分钟安装，立刻见效）→ Bash 输出压缩 60-90%，成本最低的 quick win
2. **Context Mode**（10 分钟安装）→ MCP 工具输出压缩 ~98%，中等复杂度
3. **CodeGraph**（15 分钟安装 + per-project init）→ 代码库索引需要时间，但一旦建好加速明显

**关键点**: 三个工具各司其职互不冲突 — 读代码时 Claude Code 自动改用 CodeGraph，调用 MCP 时通过 Context Mode 执行，调用外部命令时由 RTK 处理。Headroom 作为第四层，覆盖前三者未处理的 RAG 块、对话历史和自定义工具输出。
...
**定位**: AI Agent 全链路输入压缩层。在工具输出、日志、RAG 结果、文件和对话历史发给 LLM 之前完成压缩，答案质量几乎不变，Token 大幅降低。

**与 Context Mode / RTK 的区别**：

| 工具 | 压缩层 | 原理 |
|------|--------|------|
| Context Mode | MCP 工具输出 | BM25 精准匹配后返回 |
| RTK | Bash 命令输出 | 100+ 内置过滤器 |
| **Headroom** | 全链路（工具输出/日志/RAG/对话历史） | 内容类型路由 + CCR 可逆压缩 |

三者互补：Headroom 覆盖其他两个未覆盖的 RAG 块、对话历史和自定义工具输出。
...
**核心特性**:
- **ContentRouter**: 自动识别 JSON/代码/日志/文本，选择最佳压缩器
- **CCR（可逆压缩）**: 原始内容本地留存，LLM 可随时通过 `headroom_retrieve` 取回
- **跨 Agent 记忆**: Claude、Codex、Gemini 等共享存储，自动去重
- **MCP 原生**: 提供 `headroom_compress`、`headroom_retrieve`、`headroom_stats` 工具
- **`headroom learn`**: 从失败会话中挖掘经验，自动写入 CLAUDE.md / AGENTS.md
...
```bash
headroom wrap claude                 # 包装 Claude Code（推荐起点）
headroom proxy --port 8787           # 零代码改动代理模式
headroom mcp install                 # MCP 原生模式
headroom perf                        # 查看 token 节省统计
```
...
```bash
headroom perf                        # 应显示 token 节省率
```

#### 主动推荐触发场景

Agent 会话中工具输出/RAG 结果/日志大量堆积，且 Context Mode 和 RTK 均已安装时，推荐 Headroom 作为第三层覆盖剩余输入。

---

### 6.2 Tokscale

**项目**: [tokscale-ai/tokscale](https://github.com/tokscale-ai/tokscale)

**定位**: 跨 20+ AI 编程工具的 token 用量统一仪表盘。扫描本地各工具的使用记录（Hermes `~/.hermes/`、Claude Code `~/.claude/projects/`、Cursor、Codex 等），汇总成统一 TUI 视图。

**与 claude-tap 的区别**：claude-tap 深度分析单次 API 请求的完整上下文；Tokscale 横向汇总多工具的历史用量，用于长期成本追踪。

**核心特性**:
- GitHub 绿格子风格的日历热力图（2D/3D）
- Overview 屏：总 Token、成本估算、高消耗模型排行
- 全本地，不登录也能完成分析
- 支持按客户端过滤、导出 JSON
...
```bash
# 零安装，直接运行（需 Node/Bun/Deno 任一）
npx tokscale@latest

# 指定客户端
tokscale --client claude-code
...
- 成本是 API 等价估算，订阅制用户（Claude Max / Cursor Pro）实际费用通常低于估算值
- 适合横向对比"谁最费钱"，不适合直接当账单
...
**项目**: [camoufox/camoufox](https://github.com/camoufox/camoufox)

**定位**: 专为 AI Agent 设计的反指纹 Firefox，在浏览器层做指纹伪装和去臃肿优化，兼容 Playwright API。被站点识别为机器人的概率比裸 Playwright 更低。

**核心特性**:
- 基于 Firefox 改造，浏览器层指纹伪装（Canvas/WebGL/字体/时区等）
- 内置 GeoIP 支持（`[geoip]` 扩展包）
- 完全兼容 Playwright，现有脚本基本无需修改
- 适合个人 Agent、研究和轻量自动化场景

#### 安装

```bash
pip install -U "camoufox[geoip]"
python -m camoufox fetch          # 下载 Firefox 二进制（一次性）
```
...
- 不是万能绕过器，风控持续进化；速率控制和合规检查仍需自行处理
- 生产级爬虫场景建议叠加代理 IP 轮换
- 适合场景：Agent 网页调研、竞品监控、自动化测试、表单填写
