...
- ✅ 人类做决策，AI 做分析
- ✅ **受众建模优先**：设计方案前，先明确"谁会读这段代码/用这个API"

**受众建模方法论**（从 market-insight Skill 提炼）：

每个设计决策都应该回答：**这个方案是给谁用的？他们的认知水平如何？**

| 受众类型 | 认知水平 | 设计侧重 | 示例场景 |
|---------|---------|---------|---------|
| **新手开发者** | 不熟悉业务逻辑，需要明确指引 | 清晰命名、充分注释、防御性编程 | 开源库、公共 SDK、团队新人维护的模块 |
| **团队成员** | 熟悉业务，但可能不熟悉特定技术栈 | 遵循团队约定、补充业务上下文注释 | 内部服务、业务逻辑代码 |
| **未来的自己** | 3 个月后会忘记细节 | 记录"为什么这么做"而非"做了什么" | 复杂算法、临时方案、技术债 |
| **API 消费者** | 只看接口不看实现 | 契约清晰、文档完整、错误提示友好 | REST API、gRPC 服务、npm 包 |
| **运维/SRE** | 关注稳定性和可观测性 | 结构化日志、监控埋点、降级开关 | 核心服务、高并发模块 |

**使用方法**：在 Innovate 阶段，AI 必须先明确受众，再设计方案。
...
**受众分析**：
- 主要受众：团队成员（5 人后端团队，熟悉 Java/Spring，但对权限模块不熟悉）
- 次要受众：未来的新人（可能 6 个月后接手维护）
- 运维视角：需要监控权限校验失败率和性能
...
- 受众友好性：
  * ✅ 团队成员友好：改动小，不破坏现有理解
  * ⚠️ 新手需要追溯调用链（Helper 是被调用的）
  * ⚠️ 可观测性一般：监控需要在两个拦截器都埋点
- ...

### 推荐：方案 A
- 原因：虽然改动面大，但一步到位，避免长期维护两套逻辑...
- 受众匹配度：团队成员可接受，对新人更友好，运维易监控
```
...
- **受众不明确时必须询问用户**：如果无法判断受众类型，AI 应主动问"这段代码主要由谁维护？"
