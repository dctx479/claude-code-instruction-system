...
> 灵感来源：MemOS 记忆插件的"分析→生成"双步流水线。一步到位的写入容易制造冗余，先分析再生成质量明显更高。

**Step 1 — 分析（Analyze）**

写入新条目**之前**，先做以下分析（不写文件）：

| 检查项 | 操作 |
|--------|------|
| 关联挖掘 | Grep 已有条目，找出语义/关键词相关的 1-3 条 |
| 矛盾识别 | 新事实是否与已有条目冲突？是否作废了某条？ |
| 重复程度 | 与最相似条目的重叠度估算（高/中/低） |
| 抽象层级 | 这条事实属于 lesson / pattern / best-practice / rule？ |
| 跨层影响 | 是否需要同步更新 docs/、CLAUDE.md、agents/？ |
...
| 分析结论 | 写入策略 |
|----------|---------|
| 重叠度高且无新信息 | **不写入**，仅在原条目追加案例引用 |
| 重叠度中等 | **合并**到现有条目，更新内容 + 追加案例 |
| 与已有条目矛盾 | 新条目正常写入，**旧条目加 `已废弃` 标记** + 注明替代条目 ID |
| 全新事实且影响多层 | 主条目写入 lessons-learned，同时更新对应 docs/CLAUDE.md |
| 全新且独立 | 直接新建条目 |

**何时使用两步链**：
- 复盘流程中写入 lessons-learned
- 提炼 best-practice 条目时
- /neat 收尾审查中处理记忆层
- 一次摄入多条候选知识时（≥3 条）

**何时可省略**：仅追加单条小事实（如修正一个拼写错误的引用）。
...
> 与 `/neat` Skill 的关系：/neat 是任务收尾时的批量整理工具，会自动遵循上述两步链协议处理记忆层。详见 `.claude/skills/neat/SKILL.md`。
...
## Skill 投资的持久性——SOP 不会过时，工具会

> 来源: ConardLi garden-skills 开源项目（2026-06），7K Stars，实战验证

**核心论点**: AI 工具在快速更迭——模型版本、Agent 框架、平台接口都在变。但 Skill 是你把自己的工作方式一点点沉淀进 AI 的过程，沉淀的是 SOP（标准操作程序），而非工具依赖。

```
工具会过时：Claude 3.5 → Claude 4.6 → Claude 5...
模型会替换：Sonnet → Opus → 更强的新模型...
框架会迭代：旧 Agent 架构被新架构替代...

但 SOP 不会过时：
  一套成熟的数据分析 Skill，换了更强的模型后效果只会更好
  一套精心设计的代码审查 Skill，框架升级后依然有效
  一套对齐了团队最佳实践的工作流 Skill，人员更迭后仍然运转
```

**量化视角**: 花 2 小时写一个 Skill，它在后续每次触发时都节省 AI"摸索"的时间（估算每次 15-30 分钟）。触发 10 次后，投入已回本；触发 100 次后，复利效应显现。

**与三层知识架构的关系**:

| 层级 | 衰减速度 | 说明 |
|------|---------|------|
| Layer 1 原始记录 | 最快 | 具体问题的解法会随环境变化而过时 |
| Layer 2 策展知识（best-practices） | 中等 | 模式和原则的生命周期较长 |
| Layer 3 Skill / 执行规则 | 最慢 | SOP 本身稳定，只需随新发现微调 |

**实践建议**: 每次解决了一类重复性问题，优先评估是否提炼为 Skill（而非只记录在 lessons-learned 中）。Skill 是知识的最高密度存储形式，也是复利效应最强的层级。
...
- `.claude/skills/neat/SKILL.md` — 任务收尾洁癖审查 Skill（自动应用 Ingest 两步链）
