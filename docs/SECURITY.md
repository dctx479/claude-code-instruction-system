...
### 第三方 Skill/Agent 安全审查

在安装或登记第三方 Skill/Agent 前，先完成安全审查，不直接运行未审查的一键安装命令。

**审查重点**:
- 权限是否最小化：警惕同时要求 shell、网络、文件写入、浏览器控制的 Skill。
- 密钥是否安全：不得读取、上传或记录 `.env`、token、cookie、浏览器会话。
- 外联是否透明：必须明确会访问的 API、域名和上传的数据类型。
- 提示词是否安全：拒绝包含越狱、隐藏指令、绕过系统约束或规避检测的 Skill。
- 功能是否重复：与现有 Skill 重叠时，优先提炼 Gotchas/模板，而不是新增依赖。

**审查结论格式**:
```markdown
### 第三方 Skill/Agent 审查结论
- 来源：仓库/作者/版本或提交标识
- 权限：shell / 网络 / 文件写入 / 浏览器 / MCP / 其他
- 密钥：是否读取、上传、记录 token、cookie、`.env`
- 外联：访问域名、API、上传数据类型
- 提示词安全：是否包含越狱、隐藏指令、绕过系统约束或规避检测
- 功能重复：与现有 Skill/Agent 的重叠和替代方案
- 结论：通过 / 隔离验证后通过 / 拒绝
```

**处理原则**:
- 安全研究、越狱、规避检测类材料只可转化为防御性审查清单，不保存可执行攻击流程。
- 需要真实账号、浏览器登录态或 API Key 的 Skill，默认在隔离环境验证后再使用。
- 只有通过审查且会长期使用的 Skill，才登记到 `docs/SKILLS-CATALOG.md`。
