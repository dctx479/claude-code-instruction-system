...
> 每条验收标准必须指定 Done Evidence：自动化测试、命令输出、手动验证步骤或审查证据。

### 验收证据矩阵

| 验收项 | Done Evidence | 验证方式 | 通过条件 |
|--------|---------------|----------|----------|
| AC-1: [验收项] | [测试/命令输出/手动验证/审查证据] | [自动/手动/审查] | [明确判定条件] |
...
- [ ] 边界条件和异常路径都有覆盖？
- [ ] 每条标准的阈值已由**提出方/技术负责人/用户**三方之一确认？（防止 AI 自定阈值，结果不可交付）
