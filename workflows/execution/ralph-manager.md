...
状态定义:
- **INACTIVE**: 未激活，等待启动
- **RUNNING**: 正在执行任务（`active=true, round_complete=false`）
- **PAUSED**: 暂停，等待人工确认（`needs_confirmation=true`）
- **COMPLETED**: 任务成功完成（`completed=true`）
- **FAILED**: 任务失败（`fatal_error=true` 或 `max_iterations_reached=true`）

> Stop Hook 根据这些字段决定是否允许停止或继续执行。

### 3. Stop Handler (停止处理器)

处理停止事件，决定是否继续执行（由 `ralph-stop-interceptor.sh` 实现）:

```
决策优先级（从高到低）:
  ralph_active == false       → 允许停止
  completed == true           → 允许停止，写入 status=COMPLETED
  fatal_error == true         → 允许停止，写入 status=FAILED
  iteration >= max_iterations → 允许停止，写入 max_iterations_reached=true
  needs_confirmation == true  → 允许停止（等待用户确认），写入 status=PAUSED
  round_complete == true      → 递增 iteration，重置 round_complete，继续（exit 3）
  round_complete == false     → 续跑当前轮，不递增（exit 3）
```

> Stop Handler 使用 Python 进行原子 JSON 更新，避免 sed 多步操作中途失败破坏状态文件。
