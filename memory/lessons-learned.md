...
### 案例引用
[必填] 引用具体的场景、commit、文件或对话片段，确保可回溯
...
**每条经验必须满足以下标准**：
1. **案例引用**：必须关联具体的 commit hash、文件路径、错误日志或对话场景
2. **避免泛化**：禁止"应该注意"、"可能会"等模糊表述，用具体的 before/after 示例
3. **可验证性**：包含明确的验证方法或复现步骤
4. **过滤噪声**：只记录可复用的模式，不记录一次性的特殊情况
...
**正确示例**：
- ✅ "Windows 上 `command -v python3` 找到 Microsoft Store 执行别名（commit abc123），需用 `python -c` 验证可执行性"
- ✅ "statusline JSON 格式错误（见 #012），唯一可靠标志是 `total_cost_usd > 0`"
...
- `[UNVERIFIED]` — 新记录，尚未验证
- `[DEPRECATED YYYY-MM-DD]` — 已废弃，不再适用（软删除）

示例: `#hooks #windows #bash [VERIFIED]`

### 软删除机制（Deprecate）

> 来源：PilotDeck 的 deprecate/restore 模式（`docs/reports/AGENT-OS-LANDSCAPE-2026.md`）

**核心理念**：不直接删除过时经验，而是标记废弃 + 保留可恢复。

**使用方法**：
1. **标记废弃**：在条目标题添加 `[DEPRECATED YYYY-MM-DD]` 标记
2. **说明原因**：在条目末尾添加"废弃原因"段落
3. **保留历史**：不删除条目内容，便于后续查证为何废弃
4. **可恢复**：如果发现废弃错误，移除 `[DEPRECATED]` 标记并更新

**废弃模板**：
```markdown
## [YYYY-MM-DD] [DEPRECATED 2026-06-10] 条目标题 #ID
...
- **废弃日期**: 2026-06-10
- **废弃原因**: [为什么这条经验不再适用]
- **替代方案**: [新的推荐做法，或关联新条目 #XXX]
- **废弃者**: [谁决定废弃]
```

**废弃场景**：
- 工具升级导致方法过时（如旧版 API 废弃）
- 发现更优解决方案（旧方案虽可用但不推荐）
- 场景变化导致不再适用（如 Windows 11 修复了 Windows 10 的某个 bug）
- 经验被证明错误（但保留以警示）

**vs 直接删除**：
- `delete` 适用于：错误记录、重复条目、无价值噪声
- `deprecate` 适用于：曾经正确但现在过时、有历史参考价值
...
## [2026-03-11] 产品设计系统案例学习：半自动复盘与知识质量控制 #014

### 问题描述
审阅外部产品经理分享的 AI 辅助产品设计系统（GitHub: product-design-system），发现其知识沉淀机制在质量控制和人机协作方面有独特优势，值得融入太一元系统。
...
- 提炼 3-5 条候选知识条目
- **人工确认**后才写入知识库
- 每条知识必须引用实际案例（HTML 原型、需求文档、变更日志）

**太一元系统现状**：
- 自进化协议是全自动触发（见 CLAUDE.md 第一章）
- 缺少人机确认环节
- 可能记录错误或噪声信息

**差距**：半自动机制通过人工审核保证知识质量，避免 AI 自我强化错误模式。

#### 2. 知识库分层索引
**外部系统做法**：
- 8 层知识库：原型规范、设计原则、交互模式、术语表
- 各管各的，AI 做任务时精准调用
- 类似"文件夹即系统"的理念

**太一元系统现状**：
- `memory/*.md` 是扁平化的（lessons-learned、best-practices、error-patterns）
- 缺少索引机制，AI 需要全文搜索
- Skills 系统有 INDEX.md，但 memory 系统没有

**差距**：分层索引可以减少 token 消耗，提高检索精度。
...
**太一元系统现状**：
- 主要依赖 Git commit history
- 文档内部无变更标记
- 难以快速定位"这条规则是什么时候、为什么加的"

**差距**：内嵌变更标记可以提高知识的可理解性和可维护性。

#### 4. 质量控制四原则
1. **必须有实际案例支撑**：不记录"应该注意"这类抽象建议
2. **避免过于泛化**：用 before/after 示例而非模糊描述
3. **过滤噪声信息**：只记录可复用的模式
4. **可回溯到具体场景**：每条知识关联 commit/文件/对话

### 融入策略

#### 已实施
1. ✅ 更新 `memory/lessons-learned.md` 使用说明，增加"案例引用"必填字段
2. ✅ 添加质量控制原则和反模式清单
3. ✅ 明确禁止记录无案例支撑的经验

#### 待实施（优先级：中）
1. **半自动复盘流程**：
   - 复杂任务完成后，AI 主动询问"是否需要复盘"
   - 提炼 3-5 条候选知识，展示给用户
   - 用户确认后写入 lessons-learned.md
   - 实现方式：在 autopilot/ralph 结束时触发

2. **Memory 索引机制**：
   - 创建 `memory/INDEX.md`，类似 Skills INDEX
   - 按主题分类：hooks、agents、statusline、配置、跨平台
   - 每个主题列出相关的经验条目 ID

3. **变更标记规范**（可选）：
   - 在 CLAUDE.md 等核心文档中用 🟢 标记新增内容
   - 用 🟡 标记修改内容
   - 保留 1-2 个版本的标记，之后移除

### 案例引用
- 外部项目：GitHub vigorX777/product-design-system
- 核心文件：AGENTS.md（员工手册）、8 层知识库结构
- 关键机制：半自动复盘、变更日志、质量控制四原则

### 验证方法
1. ✅ 检查 lessons-learned.md 是否已更新质量控制原则
2. 下次完成复杂任务时，尝试主动询问用户"是否需要复盘"
3. 观察新增的经验条目是否都包含"案例引用"字段
...
- **质量保证**：人工审核过滤噪声和错误
- **学习机会**：用户在确认过程中也在学习
- **信任建立**：用户知道知识库内容是经过审核的

#### 为何需要案例引用
- **可验证性**：可以回溯到具体场景验证真实性
- **可理解性**：新用户可以通过案例理解规则的来源
- **可维护性**：当规则过时时，可以追溯到原始场景判断是否仍然适用

#### 道家智慧映射
- **半自动复盘** → 无为而治：系统提供候选，人做最终决策
- **案例引用** → 实事求是：知识必须源于实践，不凭空臆造
- **质量控制** → 去伪存真：过滤噪声，保留精华
