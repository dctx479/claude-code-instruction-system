...
> 来源: Harness 长时间任务概念 + 127 只科技股批量分析实践（lessons-learned #015）

### 模式选择指南

| 模式 | 适用场景 | 核心策略 |
|------|---------|---------|
| **批量处理+暂停点** | >20 个同质任务（如逐个分析股票） | 每 N 项暂停审查质量 |
| **深度优先** | 新类型任务，模板未确定 | 先 1 个样本建模板，再批量 |
| **检查点恢复** | 任务可能被中断（超时/网络/手动） | progress.md 写入/读取 |

### 模式 1: 批量处理带暂停点

```
/ralph "按 task_plan.md 分析 50 只股票，每 10 只暂停审查"

工作流:
  轮次 1-10: 逐个分析前 10 只
    → 第 10 只完成 → needs_confirmation: true
    → 用户审查质量 → 确认继续
  轮次 11-20: 分析 11-20 只
    → 第 20 只完成 → needs_confirmation: true
    ...
```

**暂停间隔设置**: `--checkpoint-interval N`，N 为处理项数（非迭代数）。

**审查重点**: 格式一致性、分析深度是否漂移、约束条件是否遗忘。

### 模式 2: 深度优先处理

```
/ralph "分析科技股，先用 AAPL 建立模板"

工作流:
  Phase A: 深度处理 1 个样本（AAPL）
    → 生成完整分析 → 提炼为模板（template.md）
    → needs_confirmation: true（用户确认模板质量）

  Phase B: 用模板批量处理剩余项
    → 每轮开头重读 template.md + task_plan.md
    → 按模板格式和深度标准执行
```

**适用信号**: 首次执行该类任务、没有现成模板、质量标准尚需探索。
...
progress.md 结构:
  ## 进度
  - [x] Item 1 — 已完成 (2026-04-15 10:00)
  - [x] Item 2 — 已完成 (2026-04-15 10:05)
  - [ ] Item 3 — 进行中
  - [ ] Item 4
  ...

恢复流程:
  1. 读取 progress.md → 找到最后完成项
  2. 从下一项继续 → 无需重新处理已完成项
  3. 验证已完成项的 findings.md 条目完整性
```

**写入时机**: 每完成一项立即更新 progress.md（而非轮次结束时批量更新）。

**恢复触发**: `/ralph resume` 自动检测 progress.md 并续跑。
