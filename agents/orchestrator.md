...
| 任务类型 | 输出模式 | 触发条件 |
|----------|----------|----------|
| 代码实现、文件修改、执行操作 | **Mode A** (执行进度式) | 有具体输出物（文件、代码、命令） |
| 分析、设计、评审、问答 | **Mode B** (分析响应式) | 输出为判断、建议、解释 |
| 混合任务（先分析后实现） | **Mode B → Mode A** | 先给出分析，再切换到执行进度 |

---

#### Mode A：执行进度式（代码/实现任务）

适用于：代码实现、文件创建/修改、命令执行、数据库操作等
...
#### Mode B：分析响应式（问答/分析任务）

适用于：架构分析、代码审查、方案评估、技术问答、设计建议等
