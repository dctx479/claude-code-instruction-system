...
如以上与您的意图不符，请更正；如无异议，我将以此为基础展开分析。
```

**为什么需要这一步**：用户的"实现这个功能"和 AI 理解的"这个功能"可能存在 mental model 差距。这个差距不是需求写错了，而是两者在没有任何验证的情况下默认"对方理解的是同一件事"。主动声明并要求确认，是最快发现对齐偏差的方式。

**触发条件**：
- 新任务（用户第一次描述需求）
- 用户措辞模糊或有多种解读可能时
- 跨会话继续任务（确保新会话理解与旧会话一致）

**规模判断（必须首先执行）**：

Agent 必须主动报告规模判断+理由，不能由人单方面指定：

```
**任务规模判断**: [小/中/大] — 理由：[具体依据]

**复杂度评分**: [1-10] — 评分依据：
  - 技术风险: [1-5]（现有技术栈是否支持？是否有未知技术盲点？）
  - 依赖复杂度: [1-5]（模块间依赖是否清晰？是否有循环依赖？）
  - 不确定性: [1-5]（需求是否稳定？是否有未解决的技术选型？）
  总分 = 三项平均 × 2（范围 2-10）

复杂度 → 流程深度对照：
  1-3（低）    → 跳过 CodeMap，直接 sdd_bootstrap
  4-6（中）    → 功能级 CodeMap（聚焦本次涉及的链路）
  7-10（高）   → 完整 CodeMap + Context Bundle + Plan 审批
  ≥8（极高）  → 必须拆分子任务，不在单个会话中完成

**注意**: 复杂度评分不是工时估计，是风险和不确定性的度量。
两个同工时任务可能复杂度差异巨大：简单 CRUD（复杂度 2）vs 缓存一致性重构（复杂度 8）。
```
...
SDD-RIPER 的每一步都可能失败。为防止中间状态污染代码库，必须在每步执行前记录**回退快照**，并在失败时自动触发逆向清理。

| 执行阶段 | 回退时机 | 回退内容 |
|---------|---------|---------|
| Plan 阶段 | 方案审批被拒 | 丢弃方案草稿，恢复到 Research 输出 |
| Execute 阶段 | 某步执行失败 | 执行该步的逆向清理，恢复到上一步完成状态 |
| Review 阶段 | 偏差过大需返工 | 恢复到 Execute 完成状态，重新进入 Plan |

**Execute 阶段回退流程**：
```
执行中 Step N 失败（如编译错误、类型不匹配）：

1. AI 报告：🔴 Step N/N 失败，原因：[描述]
2. 不修正错误就继续 → 立即执行回退
3. 回退清单（每个操作对应一条清理）：
   - 新增文件 → `git rm <file>`
   - 修改文件 → `git checkout <file>`（恢复到上一步版本）
   - 新增配置 → 删除对应的配置条目
4. 回退完成后报告：✅ 已回退到 Step N-1 完成状态
5. 询问用户：如何处理失败原因？
   - 修正 Plan → 回到 Plan 阶段重新审批
   - 终止任务 → 输出总结，关闭 Spec
```

**回退原则**：
- ✅ 每一步执行前，AI 先确认 git commit 已记录当前快照
- ✅ 只回退失败步及其后续，保留已完成步骤的产出
- ❌ 不在同一次操作中"边修边跑"——回退和重试是分离的两个动作
- ❌ 回退不由 AI 自行判断继续执行，必须由用户确认方向
