...
**关键认知：审稿人可能不是你这个细分领域的专家。**

他们一眼能看出的：图表好不好看、英文原文顺不顺、引用格式对不对。他们需要时间判断的：科学贡献本身。

**实践要求**：
1. **图表质量**：使用语义化配色（蓝=主方法，绿=变体，红=基线），SVG 文字可编辑
2. **英文流畅度**：沙漏结构（Introduction 收窄，Discussion 展开），时态一致性审计
3. **引用规范**：Nature 系列 28 页作者指南中的引用格式必须严格遵守
4. **过度声明检测**：结论不能超出数据支撑范围，"demonstrates" vs "suggests" 的区分

> 参考: `memory/best-practices.md` BP-031（Nature 期刊标准参考）
...
**结构层**：
- [ ] 沙漏结构：Introduction 从宽到窄，Discussion 从窄到宽
- [ ] 每段首句概括主题（topic sentence）
- [ ] 各章节过渡句自然
- [ ] Abstract 字数在目标期刊限制内

**语言层**：
- [ ] 时态审计：Methods 用过去时，已知事实用现在时
- [ ] 无过度声明（"demonstrates" 仅用于强证据，弱证据用 "suggests"）
- [ ] 无 AI 写作特征词（可配合 paper-revision 去AI化模式）
- [ ] 术语全文一致

**图表层**：
- [ ] 语义化配色，同一方法跨子图同色
- [ ] 图中文字 ≥8pt，SVG/PDF 矢量导出
- [ ] 每图只传递一个核心信息
- [ ] 图表标题自含（不看正文也能理解）

**合规层**：
- [ ] 数据可用性声明（Data Availability Statement）已撰写
- [ ] FAIR 合规（可发现、可访问、可互操作、可复用）
- [ ] 利益冲突声明
- [ ] 伦理审批声明（如适用）
