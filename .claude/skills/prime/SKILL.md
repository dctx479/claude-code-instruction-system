...
在输出摘要前，Agent 自我确认上下文正确性：

```
状态确认清单：
  [ ] 我知道当前在哪个分支上（不是 main 默认）
  [ ] 最近提交与当前任务相关（不是完全不相关的冷门变更）
  [ ] 检入的上下文（intent-state.json / memory index）与用户需求匹配
  [ ] 没有混淆上一次的会话状态（当前会话 vs 上一次会话）

如发现不匹配 → 在摘要中显式说明：
  ⚠️ 分支/状态不匹配：[说明]，已从 [旧状态] 切换到 [新状态]
```

**为什么需要这一步**：避免"上下文漂移"——当前会话可能从 `/compact` 恢复，或从另一个项目切换来，Agent 需要确认自己是否处于正确上下文中。
