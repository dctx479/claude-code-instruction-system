...
4. 是否来自第三方仓库、外部模板或需要新增高权限工具？如是，先按 `docs/SECURITY.md` 完成第三方 Skill/Agent 安全审查。
...
每个 Skill 必须显式声明这四块（缺一不可），并让 description / trigger 能准确支持能力路由：

- `description` 说明适用任务、核心输出和边界，避免泛化成万能 Skill。
- `trigger` 覆盖真实用户语境，但不堆叠宽泛关键词。
- `调用时机` 同时写适用和不适用场景，帮助路由器做负向选择。
...
3. 自检清单：
   ...
   - [ ] 触发词覆盖中英双语
   - [ ] 与现有 Skill 无功能重叠
   - [ ] 第三方来源或高权限能力已引用 `docs/SECURITY.md` 审查结论
   - [ ] **Skill 测试验证**：至少用真实任务测试过一次（不是单元测试，是"这个 Skill 真的能解决实际问题"的功能验证）
