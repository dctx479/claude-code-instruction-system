...
### Agent 封装类 (Agent Packaging)

#### claude-agent-sdk
```yaml
文件: .claude/skills/claude-agent-sdk/SKILL.md
类型: Heavy（重量级）
描述: 将任何 Skill 封装为 Claude Agent SDK Web 应用，五步法 + Python/TypeScript 代码模板 + 安全部署
适用: 将已验证的 Claude Code Skill 转化为对外 Web SaaS 服务
专长:
  - Step 1: 环境准备（依赖安装 + Skill 文件定位）
  - Step 2: Skill 分析 → 工具接口映射（操作 → @tool 装饰器）
  - Step 3: Agent 核心代码（Python/TypeScript 双模板 + MCP Server 注册）
  - Step 4: FastAPI/Express Web 服务层 + WebSocket 实时对话
  - Step 5: 端到端验证（工具调用/图表生成/日志可见性/文件上传）
  - 安全部署五层防御（L1权限/L2工具白名单/L3 Hooks/L4 Docker/L5 LlamaFirewall）
核心原则: Skill 是开发核心，Agent SDK 只是封装壳；最小工具权限原则，禁止 acceptEdits 用于生产
触发: "用 claude-agent-sdk 建立 webapp"、"把 skill 转为 web 应用"、"构建 claude agent sdk 项目"
集成: [skill-creator, sdd-riper, code-security-review]
来源: liangdabiao/claudesdk-skill（15+ 已验证开源案例）
```
...
#### neat
```yaml
文件: .claude/skills/neat/SKILL.md
类型: Light（轻量级）
描述: 任务收尾洁癖审查，五步法同步 docs/CLAUDE.md/memory 三层知识体系
适用: 任务收尾、/compact 之前、autopilot/ralph 自然结束、上下文 >250K 时
专长:
  - Step 1 机械式盘点（不依赖记忆，强制 Read 全部 md 文件）
  - Step 2 影响矩阵（识别需改的层级 + 跨项目影响）
  - Step 3 按层修改（顺序：docs → CLAUDE.md → memory）
  - Step 4 自检清单（环境变量/相对时间/悬空引用/规模上限）
  - Step 5 变更摘要输出
核心原则: 合并优于追加，删除优于保留（过期记忆比无记忆更糟）
触发: /neat、"整理一下"、"审查一下文档"、"存档"、"收尾"
集成: [self-evolution, context-engineering, sdd-riper, ralph-loop, autopilot]
配套: docs/KNOWLEDGE-COMPOUNDING-GUIDE.md（Ingest 去重协议）
```

#### reflection
```yaml
文件: .claude/skills/reflection/SKILL.md
类型: Light（轻量级）
描述: 任务复盘提炼器，萃取 3-5 条可复用经验，两步链 Ingest 写入知识库，评估 Skill 提取价值
适用: 复杂任务完成后、解决重大 bug 后、autopilot/ralph 结束时
专长:
  - Step 1 萃取候选经验（踩坑/发现/决策/约束四类，强制 before/after + 案例引用）
  - Step 2 两步链 Ingest（先 Grep 分析重叠度，再按策略写入：新建/合并/废弃旧+写新）
  - Step 3 Skill 提取评估（重复模式≥3次/流程≥5步/领域知识 → 建议调用 /skill-creator）
  - Step 4 链式触发 /neat 收尾
核心原则: 宁少勿滥，质量优先；必须用户确认后才写文件
触发: /reflection、"复盘一下"、"总结经验"、"提炼教训"
集成: [neat, skill-creator, knowledge-compounding]
```
...
类型: Light（轻量级）
描述: Skill 元创建器，按项目契约（What/How/When done/What NOT）+ 单一职责 + 方法论优于规则三原则生成高质量 SKILL.md
适用: 识别出可复用流程（重复≥3次/复杂流程≥5步/领域知识）时，将经验封装为 Skill
专长:
  - Step 1 单一职责检查（无"+"号陈述，Grep 避免重复）
  - Step 2 方法论提炼（给判断框架，而非纯规则）
  - Step 3 契约四要素填充（What/How/When Done/What NOT）
  - Step 4 生成 SKILL.md 骨架（含 frontmatter + 所有必填章节）
  - Step 5 轻重双轨判断（Light ≤300行 vs Heavy 多文件）
  - Step 6 INDEX.md 注册 + 自检清单
核心原则: 方法论 > 规则；没有真实使用经验禁止生成空壳
触发: /skill-creator、"创建一个 Skill"、"把这个流程封装成 Skill"、"提炼成 Skill"
集成: [reflection, neat, knowledge-compounding]
```

#### prime
```yaml
文件: .claude/skills/prime/SKILL.md
类型: Light（轻量级）
描述: 会话预热仪式，新会话开始时快速建立项目上下文：读取 git 状态、检索相关记忆、输出状态摘要
适用: 新会话开始、/compact 之后继续工作、会话中途换任务
专长:
  - Step 1 Git 状态快照（git log -10 + status + stash + branch，~30秒）
  - Step 2 精准记忆检索（lessons-learned 最后20条 + 按需 Grep + context/index.json）
  - Step 3 状态摘要输出（≤20行：项目状态 + 相关记忆 + 建议下一步）
核心原则: 先检索再行动（MemOS 记忆检索优先原则）；不全量读取文档，精准加载
触发: /prime、"新会话开始"、"帮我回顾一下项目状态"、"我们上次做到哪了"
集成: [neat, reflection, knowledge-compounding, context-engineering]
配套: /neat（对称的会话结束仪式）；完整生命周期：/prime → 执行 → /reflection → /neat
```
...
类型: Light（轻量级）
描述: 结构化调试五步工作流，把"随机尝试"变成"系统排查"，确保每次调试都有迹可循
适用: 遇到报错/异常行为、测试失败原因不明、生产问题排查
专长:
  - Step 1 复现（稳定触发问题，禁止在无法复现时改代码）
  - Step 2 隔离（二分法定位，最小化复现用例）
  - Step 3 假设（1-3个可验证假设按可能性排序，禁止同时验证多个）
  - Step 4 验证修复（按优先级逐一验证，最小化修复原则）
  - Step 5 防复发（三问：为什么发生/为什么没发现/如何防止）
核心原则: 一次只改一个变量；没有稳定复现就不改代码
触发: /debug、"帮我调试"、"这个报错怎么回事"、"找一下 bug"、"为什么不工作"
集成: [debugger Agent, reflection, observability, memory/error-patterns.md]
```
...
核心原则: 决策点/错误/不确定性必记；不记录琐碎操作
触发: /observe、/thought、/log-decision、"记录思考过程"、"思维日志"
...
类型: Heavy（重量级）
版本: v1.1
描述: Spec-Driven Development + RIPER 五阶段流程，确保大模型编程质量可控；含标准契约四要素
适用: 中大型需求开发（>500行或>3文件）、复杂功能实现、需要知识沉淀的场景
专长:
  - Research（调研与事实锁定）
  - Innovate（方案设计与对比）
  - Plan（原子级规划）
  ...
  - Review（验收闭环）
输出: 
  - specs/<task>.md（核心 Spec 文档，§1-§8 完整）
  - codemap/<feature>.md（代码地图）
  - archive/<task>_human.md + _llm.md（知识沉淀）
触发: /riper、/sdd-riper、中大型需求开发、需要写规范文档、复杂功能实现
集成: [codemap-builder, qa-reviewer, qa-fixer, spec-first]
参考: docs/SDD-RIPER-GUIDE.md
轻重双轨: 标准版（>500行/3+文件）vs sdd-riper-light（<500行/<3文件）
```
...
类型: Light（轻量级）
版本: v1.1
描述: SDD-RIPER 轻量版，保留核心质量保障，简化流程负担；含标准契约四要素
适用: 熟练用户、简单任务（<500 行代码且 <3 个文件）、快速迭代
专长:
  - Micro-Spec（1-2 段精简文档）
  - 单方案快速决策（无需多方案对比）
  - 高层级步骤规划（3-5 步）
  - 快速验收（核心功能检查）
  - 自动判断：<3文件且<500行 → 轻量版；否则升级标准版
输出: 
  - specs/<task>-micro.md（Micro-Spec）
  - Change Log（修改摘要）
触发: /riper-light、/sdd-light、简单 bug 修复、小功能开发、快速迭代
...
#### handoff
```yaml
文件: .claude/skills/handoff/SKILL.md
类型: Light（轻量级）
版本: v1.0
描述: 上下文交接仪式，/compact 之前将当前进度、未完成决策、下一步行动结构化写入 memory/handoff.md
适用: 上下文 >250K tokens 准备 /compact 前、任务中途暂停需要隔天继续时
专长:
  - Step 1 盘点当前状态（git status + stash + 上下文推断）
  - Step 2 识别隐性上下文（约束/决策/依赖/风险，新 AI 接手会犯什么错？）
  - Step 3 写入 memory/handoff.md（≤50行，含下一步可直接执行的指令）
核心原则: git 状态是结果，交接文档是过程；/prime 读结果，/handoff 写过程
触发: /handoff、准备 compact、上下文快满了、交接一下、记录当前进度
集成: [prime, neat, reflection, context-engineering]
配套: /prime（对称的会话开始仪式）；完整生命周期：/prime → 执行 → /handoff → /neat → /compact
```

#### spec-first
```yaml
文件: .claude/skills/spec-first/SKILL.md
类型: Light（轻量级）
版本: v1.0
描述: 完整 QA 工作流编排器，串联 spec-writer → 开发实现 → qa-reviewer → qa-fixer 三步流程
适用: 新功能开发需要质量保障、重要 Bug 修复需要验收时
专长:
  - Step 1 spec-writer Agent → SPEC-{feature}.md（门禁：用户确认后才进 Step 2）
  - Step 2 开发实现（严格按 Spec，不添加 Spec 未定义功能）
  - Step 3 qa-reviewer 评分（≥80通过 / <80→P2→qa-fixer自动修复→重新评分 / P0/P1→人工修复）
核心原则: 规范是唯一事实来源，代码和测试都对齐规范
触发: /spec-first、完整 QA 流程、先写规范再开发、需要质量保障、spec first
集成: [spec-writer Agent, qa-reviewer Agent, qa-fixer Agent, sdd-riper]
升级路径: sdd-riper-light → spec-first → sdd-riper
```
...
#### task-decompose
```yaml
文件: .claude/skills/task-decompose/SKILL.md
类型: Light（轻量级）
版本: v1.0
描述: 任务分解器，将模糊需求拆成原子任务列表，每个任务可独立执行、可验收、有明确输入输出
适用: 需求拆解、Sprint 规划、多人协作任务分配、依赖关系梳理
专长:
  - 三层结构拆解（Epic → Story → Task，单任务 ≤ 4 小时）
  - 依赖图生成（ASCII 格式，标注并行路径和关键路径）
  - 任务类型标签（BE/FE/DB/TEST/DOC/INFRA）
  - 风险任务标记（技术不确定性高的任务 ⚠️）
  - 并行化建议（哪些任务可同时进行）
输出: 原子任务列表 + 依赖图 + 关键路径 + 估时
触发: /task-decompose、任务拆解、需求分解、拆分任务、原子任务、任务依赖图
集成: [sdd-riper, spec-first, parallel-explore, handoff]
```

#### pr-prep
```yaml
文件: .claude/skills/pr-prep/SKILL.md
类型: Light（轻量级）
版本: v1.0
描述: PR 提交前结构化检查仪式，五步检查确保每个 PR 都能一次通过 Review
适用: 功能开发完成准备提 PR 时、代码合并前的最后检查
专长:
  - Step 1 分支状态检查（未提交变更/WIP commit/commit 规范）
  - Step 2 代码质量检查（调试语句/TODO/硬编码密钥）
  - Step 3 测试覆盖检查（测试通过/新功能有测试/覆盖率）
  - Step 4 文档同步检查（README/API 文档/CHANGELOG）
  - Step 5 PR 描述生成（Summary + Changes + Test Plan + Impact）
输出: 结构化检查报告（🔴阻塞/🟡警告/✅通过）+ PR 描述草稿
触发: /pr-prep、PR 准备、提交 PR 前、PR 检查清单、准备合并
集成: [code-reviewer, sdd-riper, spec-first]
```
...
| **部署安全** | code-security-review | 公开部署安全评估 |
| **Skill → SaaS** | claude-agent-sdk | 将 skill 封装为 Web 对外服务 |
| **复盘** | reflection | 任务完成后萃取可复用经验，写入知识库 |
| **收尾** | neat | 任务结束时同步三层知识体系，防止文档腐败 |
| **Skill化** | skill-creator | 识别出高频/复杂流程时，将经验封装为 Skill |
| **会话预热** | prime | 新会话开始时快速建立项目上下文，先检索再行动 |
| **调试** | debug | 遇到报错/异常行为时，五步系统排查 |
| **上下文交接** | handoff | /compact 前将进度/决策/下一步写入交接文档 |
| **QA 流程** | spec-first | 新功能开发时串联 spec-writer→开发→qa-reviewer→qa-fixer |
| **任务拆解** | task-decompose | 需求模糊或多人协作时，拆成原子任务 + 依赖图 |
| **PR 提交** | pr-prep | 提 PR 前五步检查，确保一次通过 Review |
| **Skill → SaaS** | claude-agent-sdk | 五步法将 skill 封装为 Web 对外服务 |

### 现象驱动路由（我看到 X，该用哪个 Skill？）

> 当你不确定该用哪个 Skill 时，从你观察到的**现象**出发查这张表。

| 我观察到… | 推荐 Skill | 说明 |
|-----------|-----------|------|
| 文档和代码对不上，AI 行为越来越奇怪 | **neat** | 三层知识失同步，需要全面审查 |
| 刚完成一个复杂任务，想把经验留下来 | **reflection** | 萃取经验 → 两步链写入 lessons-learned |
| 同一个流程已经做了 3 次以上 | **skill-creator** | 高频流程值得封装成 Skill |
| 研究问题太模糊，不知道从哪里搜 | **question-refiner** | 先精炼查询，再调用 deep-research |
| 需要对比多个技术方案，不知道选哪个 | **parallel-explore** | 独立 worktree 实现每个方案，标准化评估 |
| 需求描述很模糊，不知道要做什么 | **vision-builder** | 引导式提问，输出 VISION.md |
| 计划写好了，想检查有没有漏洞 | **plan-review** | 10 维度评估，识别风险 |
| 任务快结束了，想确保知识不丢失 | **reflection** → **neat** | 先复盘提炼，再三层同步 |
| 上下文快满了（>250K tokens） | **neat** → /compact | 先同步知识，再压缩 |
| 新会话开始，不知道上次做到哪了 | **prime** | 读取 git 状态 + 检索记忆，建立上下文 |
| 遇到报错/异常行为，不知道从哪下手 | **debug** | 五步系统排查，一次只改一个变量 |
| 上下文快满了，准备 /compact | **handoff** → **neat** → /compact | 先写交接文档，再同步知识，再压缩 |
| 新功能开发，想确保质量有保障 | **spec-first** | 规范驱动：先 Spec → 开发 → QA 验收循环 |
| 需求模糊，不知道怎么拆任务 | **task-decompose** | 三层拆解 + 依赖图 + 并行化建议 |
| 功能写完了，准备提 PR | **pr-prep** | 五步检查：测试/调试代码/文档/commit/PR 描述 |
| 想把 skill 对外开放成 Web 服务 | **claude-agent-sdk** | 五步法封装 skill 为 SaaS，含安全部署指南 |
...
### 2026-04-30
- 新增规划类 Skills：`task-decompose`（任务分解器）、`pr-prep`（PR 准备仪式）
- `task-decompose`：三层拆解（Epic→Story→Task）+ 依赖图 + 并行化建议，单任务 ≤ 4 小时
- `pr-prep`：五步检查仪式（分支状态/代码质量/测试覆盖/文档同步/PR 描述），输出 🔴🟡✅ 分级报告
- 批量补 trigger frontmatter（第二批 15 个 Skill）：seedance-prompt/seedance-storyboard/paper-revision/amazon-analyse/brightdata-research/social-media-research/collaborating-with-codex/collaborating-with-gemini/frontend-design/god-oversight/literature-mentor/react-best-practices/web-artifacts-builder/ui-ux-pro-max/schedule-analyzer
- 场景速查表新增"任务拆解"和"PR 提交"两个阶段
- 现象驱动路由新增 2 条：需求模糊不知怎么拆 → task-decompose；功能写完准备提 PR → pr-prep
- 新增系统类 Skills：`handoff`（上下文交接）、`spec-first`（完整 QA 工作流）
- `handoff`：三步法写交接文档（盘点状态→识别隐性上下文→写入 memory/handoff.md），填补 /compact 前的生命周期空白
- `spec-first`：串联 spec-writer→开发→qa-reviewer→qa-fixer 三步 QA 闭环，规范是唯一事实来源
- `sdd-riper` 升级至 v1.1：补充契约四要素（What/How/When Done/What NOT）+ trigger frontmatter（/riper、/sdd-riper 等）
- `sdd-riper-light` 升级至 v1.1：补充契约四要素 + trigger frontmatter（/riper-light、/sdd-light 等）+ 明确轻重判断阈值（<3文件且<500行）
- 批量补 trigger frontmatter（第一批 7 个 Skill）：question-refiner/market-insight/vision-builder/exa-research/parallel-explore/deep-research/plan-review
- 场景速查表新增"上下文交接"和"QA 流程"两个阶段
- 现象驱动路由新增 2 条：上下文快满了 → handoff→neat→/compact；新功能开发 → spec-first
- 新增系统类 Skills：`prime`（会话预热）、`debug`（结构化调试）
- `prime`：三步法建立会话上下文（git快照+精准记忆检索+状态摘要），MemOS 记忆检索优先原则
- `debug`：五步调试工作流（复现→隔离→假设→验证修复→防复发），核心原则"一次只改一个变量"
- `observability` 升级至 v1.1：补充契约四要素（What/How/When Done/What NOT）+ trigger frontmatter
- 场景速查表新增"会话预热"和"调试"两个阶段
- 现象驱动路由新增 2 条：新会话开始 → prime；遇到报错 → debug
- 新增系统类 Skills：`reflection`（任务复盘提炼器）、`skill-creator`（Skill 元创建器）
- `reflection`：四步法萃取经验 + 两步链 Ingest 去重 + Skill 提取评估 + 链式触发 /neat
- `skill-creator`：六步法创建高质量 Skill，强制契约四要素 + 单一职责 + 方法论优于规则
- 场景速查表新增"复盘"和"Skill化"两个阶段
- 新增"现象驱动路由"速查表（从观察到的现象出发找 Skill）
